...
public class GpsInfoActivity extends USBGpsBaseActivity implements
    ...
    private void setupUI() {
        ...
        detailsbutton = (AppCompatButton) findViewById(R.id.more_button);
    ...
    private void updateInfoData() {
        if (!isDoublePanel()) {
            startSwitch.setChecked(isProviderRunning());
        }
        ...
        // Details should only be accessable when the service is running
        detailsbutton.setEnabled(isProviderRunning());
    }
    ...
    private boolean canShowMoreDetails() {
        boolean sirfEnabled = sharedPreferences.getBoolean(getString(R.string.pref_sirf_gps_key), false);
        boolean gsvEnabled = sharedPreferences.getBoolean(getString(R.string.pref_sirf_enable_gsv_key), false);
        return sirfEnabled && gsvEnabled;
    }

    private void onMoreDetailsClicked() {
        if (!canShowMoreDetails()) {
            new AlertDialog.Builder(this)
                    .setTitle(R.string.details_not_available_title)
                    .setMessage(R.string.details_not_available_summary)
    ...
    public void onResume() {
        ...
        application.registerServiceDataListener(this);
        super.onResume();
    }
    ...
    public void onPause() {
        ...
        application.unregisterServiceDataListener(this);
        super.onPause();
    }
